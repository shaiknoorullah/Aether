...
**Risk annotation**: every registry entry — builtin or user — carries a `risk` class (`read` / `navigate` / `mutate-local` / `mutate-remote` / `dangerous`) **and an `agent` flag** (default `true`). Both are unused in v1.3 beyond display; they exist now so v2.1's consent policy annotates commands *at authoring time* rather than retrofitting a hundred of them. `agent` is in the frozen entry shape from the start specifically so a1 does not have to break a frozen contract on day one — which it would otherwise have to, since `validateCommandDef` would reject a field it doesn't know and `aether.version` would need a bump before the facade had shipped once. Builtins are annotated in this spec: `zap`/`boost` are `mutate-local`, `mod_enable` is `dangerous`, navigation is `navigate`.
